...
import { z } from 'zod';
import { Copy, Loader2, Lock, LockOpen, Trash2 } from 'lucide-react';
...
import { updateSettings, getUnmaskedSettings, removePaymentIntegration } from '@/app/settings/actions';
...
export function MercadoPagoConfigDialog({ open, onOpenChange, settings }: MercadoPagoConfigDialogProps) {
    ...
    const webhookUrl = `${getApiUrl(baseUrl)}/global/webhook/${mode === 'sandbox' ? 'sand' : ''}mercadopago`;
                                ...
                                <Button
                                    ...
                                    onClick={async () => {
                                        if (confirm('Tem certeza que deseja remover esta configuração? Isso impedirá o processamento de novos pagamentos.')) {
                                            const result = await removePaymentIntegration('mercadopago');
                                            if (result.success) {
