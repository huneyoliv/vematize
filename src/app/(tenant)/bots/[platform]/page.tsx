...
import { checkPlatformAccess } from "./check-platform-access";
...
export default async function BotPlatformPage({ params }: { params: { platform: string } }) {
    ...
    const platformAccess = await checkPlatformAccess(platformKey);

    if (!platformAccess.hasAccess) {
        return (
            <AccessDenied
                platform={platformKey}
                planName={platformAccess.planName}
                allowedPlatforms={platformAccess.allowedPlatforms}
            />
        );
    }
