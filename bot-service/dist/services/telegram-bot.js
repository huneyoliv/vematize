...
const pushinpay_service_1 = require("./pushinpay.service");
...
function replacePlaceholders(text, from) {
    if (!text)
        return '';
...
async function executeStep(ctx, step) {
    ...
    if (mediaMatch) {
        ...
        const botServiceUrl = env_1.env.BASE_URL;
...
async function generateProfileMessage(db, user, options = {}) {
...
function createBotInstance(token) {
    ...
    bot.use(async (ctx, next) => {
        if (ctx.tenant) {
            ...
            if (!(0, subscription_1.hasValidSubscription)(ctx.tenant)) {
                const expiredMessage = (0, subscription_1.getExpiredSubscriptionMessage)(ctx.tenant);
                if (ctx.callbackQuery) {
        ...
        const tenant = await db.collection('tenants').findOne({ "connections.telegram.botToken": token });
        if (!tenant) {
        ...
        const parseResult = schemas_1.BotConfigSchema.safeParse(tenant.botConfig);
        if (!parseResult.success) {
            ...
            tenant.botConfig = parseResult.data;
        }
        // Valida assinatura
        if (!(0, subscription_1.hasValidSubscription)(tenant)) {
            ...
            const expiredMessage = (0, subscription_1.getExpiredSubscriptionMessage)(tenant);
            if (ctx.callbackQuery) {
                await ctx.answerCbQuery(expiredMessage);
            }
            else {
    ...
    bot.on('text', async (ctx) => {
        ...
        await usersCollection.updateOne({ telegramId: ctx.from.id, tenantId: tenant._id.toString() }, {
        ...
        const flow = botConfig.flows.find((f) => f.trigger === command);
        if (flow) {
            ...
            if (command === '/perfil') {
                const mainFlow = botConfig.flows.find((f) => f.trigger === '/start');
    ...
    bot.on('callback_query', async (ctx) => {
        ...
        switch (actionType) {
            ...
            case 'LINK_TO_PRODUCT':
                ...
                const product = await productsCollection.findOne({ tenantId: tenant._id.toString(), _id: new mongodb_1.ObjectId(payload) });
            ...
            case 'REMOVE_COUPON':
                ...
                await db.collection('sales').updateOne({ _id: new mongodb_1.ObjectId(saleIdForRemove) }, { $unset: { couponCode: "" } });
                await ctx.answerCbQuery('Cupom removido!');
                break;
            case 'ACQUIRE_PRODUCT':
                if (!payload)
                    return ctx.answerCbQuery('Produto não especificado.');
                const productToAcquire = await db.collection('products').findOne({ _id: new mongodb_1.ObjectId(payload) });
                if (!productToAcquire)
                    return ctx.answerCbQuery('Produto não encontrado.');
            ...
            case 'BUY_WITH_METHOD':
                ...
                const productForPurchase = await db.collection('products').findOne({ _id: new mongodb_1.ObjectId(productId), tenantId: tenant._id.toString() });
                ...
                if (gateway === 'mercadopago') {
                    if (method === 'credit_card') {
                        ...
                        const result = await (0, mercadopago_service_1.createMercadoPagoPreference)(tenant, productForPurchase, saleId, buyerId, finalPrice);
                        if (result.success && result.init_point && result.preferenceId) {
                            await salesCol.updateOne({ _id: new mongodb_1.ObjectId(saleId) }, { $set: { "paymentDetails.init_point": result.init_point, "paymentDetails.preferenceId": result.preferenceId } });
                        ...
                        if (saleForPurchase.paymentDetails?.qrCode && saleForPurchase.paymentDetails?.qrCodeBase64) {
                            ...
                            await salesCol.updateOne({ _id: new mongodb_1.ObjectId(saleId) }, { $set: { telegramMessageId: photoMessage.message_id } });
                            return;
                        }
                        const result = await (0, mercadopago_service_1.createMercadoPagoPixPayment)(tenant, productForPurchase, saleId, buyerId, finalPrice);
                        if (result.success && result.qrCode && result.qrCodeBase64 && result.paymentId) {
                            await salesCol.updateOne({ _id: new mongodb_1.ObjectId(saleId) }, {
                                $set: {
                    ...
                    if (method === 'credit_card') {
                        ...
                        const result = await (0, pushinpay_service_1.createPushinPayCheckout)(tenant, productForPurchase, saleId, buyerId, finalPrice);
                        if (result.success && result.checkoutUrl) {
                            await salesCol.updateOne({ _id: new mongodb_1.ObjectId(saleId) }, { $set: { "paymentDetails.init_point": result.checkoutUrl, "paymentDetails.paymentId": result.paymentId } });
                            await ctx.editMessageText(`✅ Link de pagamento gerado! Valor: R$ ${finalPrice.toFixed(2).replace('.', ',')}\nClique no botão abaixo para pagar.`, {
                                reply_markup: {
                                    inline_keyboard: [
                        ...
                        const result = await (0, pushinpay_service_1.createPushinPayPixPayment)(tenant, productForPurchase, saleId, buyerId, finalPrice);
                        if (result.success && result.qrCode && result.qrCodeBase64 && result.paymentId) {
                            await salesCol.updateOne({ _id: new mongodb_1.ObjectId(saleId) }, {
                                $set: {
                                    "paymentDetails.qrCode": result.qrCode,
                    ...
                    const result = await (0, stripe_service_1.createStripeCheckoutSession)(tenant, productForPurchase, saleId, buyerId, finalPrice);
                    if (result.success && result.checkoutUrl) {
                        await salesCol.updateOne({ _id: new mongodb_1.ObjectId(saleId) }, { $set: { "paymentDetails.init_point": result.checkoutUrl, "paymentDetails.sessionId": result.sessionId } });
