...
import { getSaasPlans } from '@/app/settings/actions';
import type { Coupon, SaasPlan } from '@/lib/types';
...
export function CouponFormDialog({ open, onOpenChange, coupon, onSuccess }: CouponFormDialogProps) {
  ...
  useEffect(() => {
    if (open && coupon) {
      form.reset({
        ...
        durationType: (coupon as any).durationType || 'forever',
        durationMonths: (coupon as any).durationMonths,
      });
    } else if (open) {
      form.reset({
        code: '',
  ...
  const onSubmit = async (data: CouponFormData) => {
    ...
    if (data.durationType) formData.append('durationType', data.durationType);
    if (data.durationMonths) formData.append('durationMonths', data.durationMonths.toString());
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Duração do Desconto</FormLabel>
                    <Select onValueChange={field.onChange} value={field.value}>
                      <FormControl>
                        <SelectTrigger>
                      ...
                      <SelectContent>
                        <SelectItem value="forever">Para Sempre (Assinatura)</SelectItem>
                        <SelectItem value="once">Uma Vez (Mensalidade Única)</SelectItem>
              ...
              render={() => (
                <FormItem>
                  <div className="mb-4">
                    ...
                    <FormDescription>
                      Selecione os planos onde este cupom pode ser aplicado. Deixe vazio para todos.
                    </FormDescription>
                  </div>
