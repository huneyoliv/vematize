...
import { getCurrentPlanInfo, getAvailablePlans, createSubscriptionPayment, CurrentPlanInfo, getPendingSubscription, checkSubscriptionStatus, cancelOldPendingSubscriptions } from "./actions";
...
export default function ClientPlanPage() {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            // 🧹 LIMPEZA: Cancela subscriptions pendentes antigas quando o usuário volta para a página
            try {
                const cleanup = await cancelOldPendingSubscriptions();
                if (cleanup.success && cleanup.cancelledCount > 0) {
                    console.log(`[Cleanup] Cancelled ${cleanup.cancelledCount} old pending subscriptions`);
                }
            } catch (error) {
                console.error('[Cleanup] Error cancelling old subscriptions:', error);
            }
            try {
    ...
    const handleManageSubscription = (action: 'change_plan' | 'update_payment') => {
        if (action === 'change_plan') {
            scrollToPlans();
        } else if (action === 'update_payment') {
            if (!planInfo?.planId) return;
            const currentPlan = availablePlans.find(p => p.id === planInfo.planId);
            if (currentPlan) {
                setSelectedPlan(currentPlan);
                setIsPaymentDialogOpen(true);
            } else {
                toast({ variant: 'destructive', title: 'Erro', description: 'Plano atual não encontrado na lista de planos disponíveis.' });
            }
        }
    };
                    ...
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
                        ...
                        <Card className="col-span-3">
                            ...
                            <CardContent className="grid gap-4">
                                <Button onClick={() => handleManageSubscription('change_plan')}>
                                    {planInfo.status === 'active' ? 'Alterar Plano' : 'Mudar de Plano'}
                                </Button>
                                <Button variant="outline" disabled={!isSubscriptionEnding && planInfo.status === 'active'} onClick={() => handleManageSubscription('update_payment')}>
                                    Atualizar Pagamento
                                </Button>
