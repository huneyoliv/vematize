...
const interactions_token_1 = require("../services/discord/interactions-token");
const logger_1 = __importDefault(require("../utils/logger"));
const router = (0, express_1.Router)();
// ===== CACHE DE PUBLIC KEYS (evita query no DB a cada PING) =====
const publicKeyCache = new Map();
const CACHE_TTL = 60000; // 1 minuto
function getCachedPublicKey(token) {
...
function setCachedPublicKey(token, publicKey, tenantId) {
 ...
 */
router.get('/interactions/:token', (0, error_handler_1.asyncHandler)(async (req, res) => {
    res.json({
        status: 'ok',
 ...
 */
router.post('/interactions/:token', rate_limiter_1.webhookLimiter, auth_1.logPublicRequest, (0, error_handler_1.asyncHandler)(async (req, res) => {
    const startTime = Date.now();
    const { token } = req.params;
    const requestId = Math.random().toString(36).substring(2, 15);
    // ===== LOGS DE DEBUG: INÍCIO DA REQUISIÇÃO =====
    logger_1.default.info(`[Discord:${requestId}] ===== INCOMING REQUEST START =====`);
    logger_1.default.info(`[Discord:${requestId}] Method: ${req.method}`);
    logger_1.default.info(`[Discord:${requestId}] URL: ${req.url}`);
    logger_1.default.info(`[Discord:${requestId}] Token: ${token.substring(0, 8)}...`);
    logger_1.default.info(`[Discord:${requestId}] IP: ${req.ip}`);
    logger_1.default.info(`[Discord:${requestId}] User-Agent: ${req.headers['user-agent']}`);
    // Headers importantes
    logger_1.default.info(`[Discord:${requestId}] Headers:`, {
    ...
    try {
        // ===== 1. RATE LIMITING =====
        logger_1.default.info(`[Discord:${requestId}] Rate limiting: PASSED (webhookLimiter applied)`);
        // ===== 2. LÊ RAW BODY (CRÍTICO PARA VERIFICAÇÃO) =====
        const rawBody = req.rawBody;
        const parsedBody = req.body;
        logger_1.default.info(`[Discord:${requestId}] ===== BODY ANALYSIS =====`);
        logger_1.default.info(`[Discord:${requestId}] Raw body length: ${rawBody ? rawBody.length : 0}`);
        logger_1.default.info(`[Discord:${requestId}] Raw body (first 500 chars): ${rawBody ? rawBody.substring(0, 500) : 'EMPTY'}`);
        logger_1.default.info(`[Discord:${requestId}] Parsed body:`, parsedBody || 'EMPTY');
        // ===== 3. VALIDA HEADERS DE SEGURANÇA =====
        const signature = req.headers['x-signature-ed25519'];
        const timestamp = req.headers['x-signature-timestamp'];
        logger_1.default.info(`[Discord:${requestId}] ===== SECURITY HEADERS =====`);
        logger_1.default.info(`[Discord:${requestId}] Signature: ${signature ? signature.substring(0, 16) + '...' : 'NULL'}`);
        logger_1.default.info(`[Discord:${requestId}] Timestamp: ${timestamp || 'NULL'}`);
        // ===== 4. BUSCA PUBLIC KEY (com cache) =====
        logger_1.default.info(`[Discord:${requestId}] ===== PUBLIC KEY LOOKUP =====`);
        let publicKey;
        let tenantId;
        let cacheHit = false;
        ...
        if (cached) {
            ...
            cacheHit = true;
            logger_1.default.info(`[Discord:${requestId}] Cache HIT - Public key found for tenant: ${tenantId}`);
        }
        else {
            logger_1.default.info(`[Discord:${requestId}] Cache MISS - Querying database...`);
            // Valida token no banco e busca tenant
            const tenant = await (0, interactions_token_1.validateInteractionsToken)(token);
            if (!tenant) {
                logger_1.default.error(`[Discord:${requestId}] ❌ Invalid token - No tenant found`);
                const response = { error: 'Invalid token' };
                logger_1.default.info(`[Discord:${requestId}] Response (401):`, response);
                return res.status(401).json(response);
            }
            tenantId = tenant._id.toString();
            const discordConnection = tenant.connections?.discord;
            logger_1.default.info(`[Discord:${requestId}] Tenant found: ${tenantId}`);
            logger_1.default.info(`[Discord:${requestId}] Discord connection exists: ${!!discordConnection}`);
            logger_1.default.info(`[Discord:${requestId}] Public key configured: ${!!discordConnection?.publicKey}`);
            if (!discordConnection?.publicKey) {
                logger_1.default.error(`[Discord:${requestId}] ❌ No public key configured for tenant: ${tenantId}`);
                const response = { error: 'Bot not configured' };
                logger_1.default.info(`[Discord:${requestId}] Response (400):`, response);
                return res.status(400).json(response);
            }
            publicKey = discordConnection.publicKey;
            logger_1.default.info(`[Discord:${requestId}] Public key loaded from database`);
            // Salva no cache
            setCachedPublicKey(token, publicKey, tenantId);
            logger_1.default.info(`[Discord:${requestId}] Public key saved to cache`);
        }
        logger_1.default.info(`[Discord:${requestId}] Cache status: ${cacheHit ? 'HIT' : 'MISS'}`);
        logger_1.default.info(`[Discord:${requestId}] Public key preview: ${publicKey.substring(0, 16)}...`);
        // ===== 5. VALIDAÇÃO DE SEGURANÇA =====
        logger_1.default.info(`[Discord:${requestId}] ===== SECURITY VALIDATION =====`);
        // Se não tem signature/timestamp, pode ser uma requisição de validação do Discord
        if (!signature || !timestamp) {
            logger_1.default.warn(`[Discord:${requestId}] ⚠️ Missing security headers - checking for validation PING`);
            // Tenta parsear o body mesmo sem assinatura (para validação inicial)
            if (parsedBody && parsedBody.type === 1) {
                logger_1.default.info(`[Discord:${requestId}] ✅ Validation PING detected (type: 1)`);
                const response = { type: 1 };
                logger_1.default.info(`[Discord:${requestId}] Response (200):`, response);
                logger_1.default.info(`[Discord:${requestId}] Processing time: ${Date.now() - startTime}ms`);
                logger_1.default.info(`[Discord:${requestId}] ===== REQUEST END =====\n`);
                return res.json(response);
            }
            logger_1.default.error(`[Discord:${requestId}] ❌ Missing security headers and no valid PING`);
            const response = { error: 'Missing security headers' };
            logger_1.default.info(`[Discord:${requestId}] Response (401):`, response);
            return res.status(401).json(response);
        }
        // ===== 6. VALIDA ASSINATURA (OBRIGATÓRIO PARA REQUISIÇÕES REAIS) =====
        logger_1.default.info(`[Discord:${requestId}] ===== SIGNATURE VALIDATION =====`);
        if (!rawBody) {
            logger_1.default.error(`[Discord:${requestId}] ❌ Empty raw body`);
            const response = { error: 'Empty body' };
            logger_1.default.info(`[Discord:${requestId}] Response (400):`, response);
            return res.status(400).json(response);
        }
        logger_1.default.info(`[Discord:${requestId}] Validating signature...`);
        logger_1.default.info(`[Discord:${requestId}] Body length for signature: ${rawBody.length}`);
        logger_1.default.info(`[Discord:${requestId}] Timestamp for signature: ${timestamp}`);
        // Usa o rawBody (string) para validação de assinatura
        let isValid = false;
        try {
            isValid = await (0, discord_interactions_1.verifyKey)(rawBody, signature, timestamp, publicKey);
        }
        catch (err) {
            logger_1.default.error(`[Discord:${requestId}] ❌ Error verifying key:`, err);
            isValid = false;
        }
        logger_1.default.info(`[Discord:${requestId}] Signature validation result: ${isValid ? 'VALID' : 'INVALID'}`);
        if (!isValid) {
            logger_1.default.error(`[Discord:${requestId}] ❌ Invalid signature`);
            const response = { error: 'Invalid request signature' };
            logger_1.default.info(`[Discord:${requestId}] Response (401):`, response);
            return res.status(401).json(response);
        }
        logger_1.default.info(`[Discord:${requestId}] ✅ Signature validated successfully`);
        // ===== 7. PROCESSA INTERACTION TYPE =====
        logger_1.default.info(`[Discord:${requestId}] ===== INTERACTION PROCESSING =====`);
        logger_1.default.info(`[Discord:${requestId}] Interaction type: ${parsedBody?.type}`);
        // ===== 7. RESPONDE AO PING (type: 1) =====
        if (parsedBody?.type === 1) {
            const responseTime = Date.now() - startTime;
            logger_1.default.info(`[Discord:${requestId}] ✅ PING interaction processed successfully`);
            logger_1.default.info(`[Discord:${requestId}] Response time: ${responseTime}ms`);
            const response = { type: 1 };
            logger_1.default.info(`[Discord:${requestId}] Response body:`, JSON.stringify(response));
            logger_1.default.info(`[Discord:${requestId}] Response Content-Type: application/json`);
            logger_1.default.info(`[Discord:${requestId}] Response Status: 200`);
            // Log final antes de enviar
            logger_1.default.info(`[Discord:${requestId}] ===== SENDING RESPONSE =====`);
            logger_1.default.info(`[Discord:${requestId}] Raw response: ${JSON.stringify(response)}`);
            logger_1.default.info(`[Discord:${requestId}] ===== REQUEST END =====\n`);
            // Resposta MANUAL e SIMPLES para garantir que nada do Express atrapalhe
            res.setHeader('Content-Type', 'application/json');
            const responseBody = '{"type":1}';
            logger_1.default.info(`[Discord:${requestId}] Sending raw response: ${responseBody}`);
            logger_1.default.info(`[Discord:${requestId}] Response Headers: ${JSON.stringify(res.getHeaders())}`);
        ...
        switch (interactionType) {
            case 3: // MESSAGE_COMPONENT (Botões, Select Menus)
                ...
                logger_1.default.info(`[Discord:${requestId}] 🔘 Handling Component Interaction: ${customId}`);
                if (customId?.startsWith('PANEL_SELECT:')) {
                    ...
                    const { tenantService } = await Promise.resolve().then(() => __importStar(require('../services/tenant.service')));
                    ...
                    logger_1.default.info(`[Discord:${requestId}] Attempting to create private channel...`);
                    logger_1.default.info(`[Discord:${requestId}] Guild: ${guildId}, User: ${userId}, BotToken: ${botToken ? 'PRESENT' : 'MISSING'}`);
                    if (botToken && guildId && userId) {
                        const sanitizedUsername = username.replace(/[^a-zA-Z0-9]/g, '-').toLowerCase();
                        const { channelService } = await Promise.resolve().then(() => __importStar(require('../services/discord/channel.service')));
                        // Usar o ID do canal onde a interação ocorreu (parsedBody.channel_id)
                        const parentChannelId = parsedBody.channel_id;
                        const thread = await channelService.createPrivateThread(parentChannelId, botToken, `cart-${sanitizedUsername}`);
                        if (thread) {
                            privateChannelId = thread.id;
                            logger_1.default.info(`[Discord:${requestId}] Private thread created: ${privateChannelId}`);
                            // Adicionar o usuário à thread
                            await channelService.addMemberToThread(thread.id, botToken, userId);
                        }
                        else {
                            logger_1.default.error(`[Discord:${requestId}] Failed to create private thread`);
                        }
                    }
                    else {
                        logger_1.default.warn(`[Discord:${requestId}] Missing required data for thread creation`);
                    }
                    ...
                    // 4. Criar Carrinho
                    const { cartService } = await Promise.resolve().then(() => __importStar(require('../services/cart.service')));
                ...
                if (customId?.startsWith('CHECKOUT:')) {
                    ...
                    (async () => {
                        try {
                            ...
                            // ------------------------------------------------------------------
                            const { saleService } = await Promise.resolve().then(() => __importStar(require('../services/sale.service')));
                            ...
                            logger_1.default.error('Error processing checkout background task:', err);
                        }
                    })();
                    return;
                }
            ...
            case 5: // MODAL_SUBMIT
                ...
                logger_1.default.info(`[Discord:${requestId}] 📝 Handling Modal Submit: ${modalId}`);
                if (modalId?.startsWith('MODAL_QUANTITY:')) {
                    ...
                    if (!currentCart)
                        return res.status(200).json({ type: 4, data: { content: '❌ Carrinho não encontrado.', flags: 64 } });
                    ...
                    const cart = await cartService.getCart(cartId);
                    if (!cart)
                        return res.status(200).json({ type: 4, data: { content: '❌ Erro ao atualizar carrinho.', flags: 64 } });
                ...
                if (modalId?.startsWith('MODAL_COUPON:')) {
                    ...
                    if (!cart)
                        return res.status(200).json({ type: 4, data: { content: '❌ Carrinho não encontrado.', flags: 64 } });
                    const { couponService } = await Promise.resolve().then(() => __importStar(require('../services/coupon.service')));
            ...
            default:
                logger_1.default.warn(`[Discord:${requestId}] ⚠️ Unknown interaction type: ${interactionType}`);
                return res.status(400).json({ error: 'Unknown interaction type' });
        }
    }
    catch (error) {
        const responseTime = Date.now() - startTime;
        logger_1.default.error(`[Discord:${requestId}] ❌ Critical error:`, {
            error: error.message,
            stack: error.stack,
