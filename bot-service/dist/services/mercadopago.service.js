...
exports.createMercadoPagoPixPayment = createMercadoPagoPixPayment;
const mercadopago_1 = require("mercadopago");
const env_1 = require("../config/env");
async function createMercadoPagoPreference(tenant, product, saleId, buyerId, finalPriceOverride) {
    try {
...
async function createMercadoPagoPixPayment(tenant, product, saleId, buyerId, finalPriceOverride) {
    try {
...
class MercadoPagoService {
    async getMpSettings(tenantId) {
        if (tenantId === 'global') {
            ...
            const tenantService = (await Promise.resolve().then(() => __importStar(require('./tenant.service')))).tenantService;
    ...
    async createSaasSubscription(tenantId, plan, customer, backUrl, reason, externalReference) {
        try {
            const { MercadoPagoConfig, PreApproval } = await Promise.resolve().then(() => __importStar(require('mercadopago')));
    ...
    async createSaasPreference(tenantId, items, payer, backUrls, externalReference, notificationUrl) {
        try {
            const { MercadoPagoConfig, Preference } = await Promise.resolve().then(() => __importStar(require('mercadopago')));
            const mpSettings = await this.getMpSettings(tenantId);
    ...
    async createSaasPixPayment(tenantId, transactionAmount, description, payer, externalReference, notificationUrl) {
        try {
            const { MercadoPagoConfig, Payment } = await Promise.resolve().then(() => __importStar(require('mercadopago')));
            const mpSettings = await this.getMpSettings(tenantId);
    ...
    async getSaasPaymentStatus(tenantId, id) {
        try {
            const { MercadoPagoConfig, Payment, PreApproval } = await Promise.resolve().then(() => __importStar(require('mercadopago')));
            const mpSettings = await this.getMpSettings(tenantId);
            const isSandbox = mpSettings.mode === 'sandbox';
            const accessToken = isSandbox ? mpSettings.sandbox_access_token : mpSettings.production_access_token;
            if (!accessToken)
