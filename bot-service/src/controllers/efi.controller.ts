...
export class EfiController {
    async createPlan(req: Request, res: Response) {
        try {
            ...
            const plan = await efiService.createPlan(tenantId, name, interval, price, repeats);
    ...
    async createSubscription(req: Request, res: Response) {
        try {
            ...
            const subscription = await efiService.createSubscription(tenantId, planId, customer, items, subscriptionId, paymentMethod);
    ...
    async handleWebhook(req: Request, res: Response) {
        try {
            ...
            await efiService.handleWebhook(req.body);
            res.status(200).send();
        } catch (error: any) {
    ...
    async registerWebhook(req: Request, res: Response) {
        try {
            ...
            const result = await efiService.registerWebhook(tenantId, webhookUrl);
            res.status(200).json({ success: true, data: result });
        } catch (error: any) {
            logger.error('Error in registerWebhook controller:', error);
            res.status(500).json({ success: false, message: error.message || 'Failed to register webhook' });
        }
    }

    async createPixCharge(req: Request, res: Response) {
        try {
            ...
            const result = await efiService.createPixCharge(tenantId, customer, items, customId, expireSeconds);
            ...
            logger.error('Error in createPixCharge controller:', error);
            res.status(500).json({ success: false, message: error.message || 'Failed to create Pix charge' });
        }
    }

    async getPixCharge(req: Request, res: Response) {
        try {
            ...
            const result = await efiService.getPixCharge(tenantId, txid);
            res.json({ success: true, data: result });
        } catch (error: any) {
            logger.error('Error in getPixCharge controller:', error);
            res.status(500).json({ success: false, message: error.message || 'Failed to get Pix charge details' });
        }
    }
}

export const efiController = new EfiController();
