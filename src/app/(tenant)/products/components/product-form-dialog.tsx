...
import { z } from 'zod';
import { FileUp, Loader2, CreditCard, QrCode, Plus, Eye } from 'lucide-react';
...
export function ProductFormDialog({ open, onOpenChange, product, tenantId, onSuccess }: ProductFormDialogProps) {
  ...
  const mediaUrlsValue = form.watch('mediaUrls');
  const codeCount = activationCodesValue?.split('\n').filter(Boolean).length ?? 0;
  const mediaCount = mediaUrlsValue?.split('\n').filter(Boolean).length ?? 0;
    ...
    if (open) {
      ...
      if (product) {
        form.reset({
          ...
          mediaUrls: Array.isArray(product.mediaUrls) ? product.mediaUrls.join('\n') : '',
                    ...
                    <FormItem>
                      ...
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="media_pack">Pack de Mídias</SelectItem>
                        </SelectContent>
                      </Select>
                      <FormMessage />
                    </FormItem>
                      ...
                      <FormItem className="animate-accordion-down">
                        ...
                        <div className="space-y-2">
                          <div className="flex gap-2">
                            <FormControl><Input placeholder="Cole a URL do arquivo ou faça upload..." {...field} value={field.value ?? ''} /></FormControl>
                            <MediaPickerDialog
                              tenantId={tenantId}
                          ...
                          <FileUploader
                            tenantId={tenantId}
                            context="digital_file"
                            onUploadComplete={(url) => field.onChange(url)}
                          />
                        </div>
                        <FormDescription>O arquivo que será enviado ao cliente após a compra.</FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  />
                ...
                {productSubtype === 'media_pack' && (
                  <>
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="animate-accordion-down">
                          <FormLabel>Capa/Preview (Imagem ou Vídeo)</FormLabel>
                          <div className="space-y-2">
                            <div className="flex gap-2">
                              <FormControl><Input placeholder="Ou cole a URL aqui..." {...field} value={field.value ?? ''} /></FormControl>
                              <MediaPickerDialog
                                tenantId={tenantId}
                                onSelect={(url) => field.onChange(url)}
                                trigger={<Button type="button" variant="outline" size="icon"><Eye className="h-4 w-4" /></Button>}
                          ...
                          <FormDescription>Mídia que será exibida na mensagem de oferta.</FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    />
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="animate-accordion-down">
                          <FormLabel>Mídias do Pack</FormLabel>
                          <div className="space-y-2">
                            <div className="flex justify-end">
                              <MediaPickerDialog
                                ...
                                trigger={<Button type="button" variant="outline" size="sm"><Plus className="w-4 h-4 mr-2" /> Selecionar da Galeria</Button>}
                              />
                            </div>
                            <FileUploader
                              tenantId={tenantId}
                            ...
                            />
                            <FormControl><Textarea placeholder="Uma URL por linha..." {...field} rows={6} /></FormControl>
                          </div>
                          <FormDescription>URLs diretas das imagens ou vídeos. Total: {mediaCount} itens.</FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    />
