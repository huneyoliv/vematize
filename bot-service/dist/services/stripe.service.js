...
exports.createStripePixPayment = createStripePixPayment;
const stripe_1 = __importDefault(require("stripe"));
const env_1 = require("../config/env");
/**
 * Cria uma sessão de checkout do Stripe para pagamento com cartão
 */
async function createStripeCheckoutSession(tenant, product, saleId, buyerId, finalPriceOverride) {
    try {
        ...
        const stripe = new stripe_1.default(secretKey, {
            apiVersion: '2025-09-30.clover', // Cast to any to avoid TS errors if version mismatch
        });
 ...
 */
async function createStripePixPayment(tenant, product, saleId, buyerId) {
    try {
        const stripeSettings = tenant.paymentIntegrations?.stripe;
        if (!stripeSettings) {
            return { success: false, message: 'O vendedor não configurou o Stripe.' };
