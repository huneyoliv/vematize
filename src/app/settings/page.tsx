...
// --- Componente do Diálogo (solução alternativa) ---
                    ...
                    <Form {...form}>
                        <form id="settings-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <FormDescription>
                                                    Encontrado nos detalhes da aplicação no painel do Mercado Pago. Essencial para a segurança.
                                                </FormDescription>
                                                <FormMessage />
                                            </FormItem>
                                        )}
                                    ...
                                    <Alert>
                                        ...
                                        <AlertDescription>
                                            <ol className="list-decimal list-inside space-y-1 mt-2">
                                                ...
                                                <li>Abaixo, você encontrará a "Chave secreta de assinatura". Copie e cole no campo acima.</li>
                                            </ol>
                                        </AlertDescription>
                                    </Alert>
                                </div>
                            ...
                            <DialogFooter className="mt-auto pt-4">
                                ...
                                <Button type="submit" form="settings-form" disabled={form.formState.isSubmitting}>
...
export default function ClientSettingsPage({ params }: { params: { subdomain: string } }) {
    ...
    const getMpStatus = () => {
        ...
        const isProductionConfigured = settings.production_public_key && settings.production_access_token;
        const isSandboxConfigured = settings.sandbox_public_key && settings.sandbox_access_token;

        if (settings.mode === 'production') {
            return isProductionConfigured 
                ? { text: 'Produção', color: 'green' as const }
                : { text: 'Não configurado', color: 'destructive' as const };
        }
        
        if (settings.mode === 'sandbox') {
            return isSandboxConfigured
                ? { text: 'Sandbox', color: 'yellow' as const }
                : { text: 'Não configurado', color: 'destructive' as const };
        }
        
        // Fallback para caso o 'mode' não esteja definido ou seja inválido
        return isProductionConfigured
            ? { text: 'Produção', color: 'green' as const }
            : isSandboxConfigured
            ? { text: 'Sandbox', color: 'yellow' as const }
    ...
    const isMpConfigured = mpStatus.text !== 'Não configurado';
        ...
        <>
            ...
            <div className="flex-1 space-y-8 p-4 md:p-8">
                ...
                <Card id="manage-payments">
                    <CardHeader>
                        <CardTitle>Gerenciar Métodos de Pagamento</CardTitle>
                    </CardHeader>
                    <CardContent>
                        {isLoading ? (
                            ...
                            <div className="flex items-center justify-between p-4 border rounded-lg">
                                <div className="flex items-center space-x-4">
                                    <MercadoPagoLogo className="h-8" />
                                    <div>
                                        <p className="font-semibold">Mercado Pago</p>
                                        <Badge variant={mpStatus.color === 'destructive' ? 'destructive' : 'outline'} className={
                                            mpStatus.color === 'green' ? 'text-green-600 border-green-600' :
                                            mpStatus.color === 'yellow' ? 'text-yellow-600 border-yellow-600' : ''
                                        }>
                                            {mpStatus.text}
                                        </Badge>
                                    </div>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Altere sua senha de acesso. Use uma senha forte para manter sua conta segura.</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <div className="space-y-2">
